...
export default function ShopReview(props) {
  ...
  const [shopId] = useState(props.shopId);
  const [userId] = useState(user["userId"]);
  const [comment, setComment] = useState("");

  useEffect(() => {
    fetch("http://127.0.0.1:8000/pp/sr/", {
      method: "GET",
      headers: {
      ...
      .then((resp) => resp.json())
      .then((result) => setShopReview(result))
  ...
  const insertComment = () => {
    if (!comment) {
      ...
      return APIService.InsertComment(
        ...
        .then((resp) => setShopReview(...shopReview, resp))
  ...
  const deleteComment = (sr) => {
    const new_sr = shopReview.filter((my_sr) => {
      if (my_sr.id === sr.id) {
        return false;
      }
      return true;
    ...
    setShopReview(new_sr);
  };

  const deleteBtn = (sr) => {
    APIService.DeleteComment(sr.id, token["mytoken"])
  ...
  const updatedComment = (sr) => {
    const new_sr = shopReview.map((my_sr) => {
      if (my_sr.id === sr.id) {
        return sr;
      } else {
        return my_sr;
  ...
  return (
    <div>
      ...
      <Row>
        ...
        >
          {shopReview
            .filter((sd) => sd.shopId === props.shopId)
            .map((sr) => {
              return (
                <div key={sr.id}>
